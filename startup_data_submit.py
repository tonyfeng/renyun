...
if __name__ == '__main__' :
    ...
    print time.strftime("%Y-%m-%d %H:%M", now_time) + '-startup_data_submit-start'
    path = os.path.abspath(os.curdir)
    fileName_consumer_error = 'reyun-consumer-error.txt'
    group_id_state = str(sys.argv[1])

    try:
        consumer = KafkaConsumer('active',group_id='reyun-data-' + group_id_state,bootstrap_servers=config.logs_source_hosts)
        for message in consumer:
            ...
            try:
                ...
                requrl = config.reyun_uri + 'startup'
        ...
        message =  time.strftime("%Y-%m-%d") + "|consumer-startup|" + ex.message
        common.write_file(path + '/logs/' + fileName_consumer_error, message, 'a')
        print message
    
    print time.strftime("%Y-%m-%d %H:%M", now_time) + '-startup_data_submit-end'
